  ...
  std::function<void(const std::vector<uint8_t> &)> filtered_callback =
      [this, data_callback](const std::vector<uint8_t> &data) {
        if (this->enabled_ || this->passive_) {
          // Fast path: skip Q31 unpack/pack when format already matches (same bit
          // depth, unit gain, all source channels selected). Common case when
          // multiple consumers share the same mono 16-bit post-AFE microphone.
          const auto &src_info = this->mic_->get_audio_stream_info();
          if (this->gain_factor_ == 1 &&
              (this->bits_per_sample_ + 7) / 8 == src_info.samples_to_bytes(1) &&
              this->channels_.count() == src_info.get_channels()) {
            data_callback(data);
            return;
          }
